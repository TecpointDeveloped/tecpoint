...
type Item = {
  ...
  extradata?: { upc?: string; color?: string };
  imagenes?: Record<string, { img?: string }>;
  issues: string[]; duplicates: string[]; publicReady: boolean;
};
type Summary = { total: number; ready: number; incomplete: number; duplicates: number };
...
export default function AdminProducts() {
  ...
  const visible=useMemo(()=>items.filter(item=>filter==="all"||(filter==="ready"&&item.publicReady)||(filter==="incomplete"&&item.issues.length)||(filter==="duplicates"&&item.duplicates.length)).filter(item=>`${item.sku} ${item.producto} ${item.extradata?.upc}`.toLowerCase().includes(query.toLowerCase())),[items,filter,query]);
  async function save(item:Item,form:HTMLFormElement){const data=new FormData(form);setBusy(true);try{await request("PATCH",{id:item.id,producto:data.get("producto"),slug:data.get("slug"),descripcion:data.get("descripcion"),categoria:data.get("categoria"),subcategoria:data.get("subcategoria"),marca:data.get("marca"),upc:data.get("upc"),color:data.get("color"),precioDetalle:data.get("precioDetalle"),precioMayoreo:data.get("precioMayoreo"),imagen:data.get("imagen")});await load();}catch(e){setError(e instanceof Error?e.message:"No fue posible guardar.");setBusy(false);}}
  async function syncW34(){setBusy(true);setNotice("");setError("");try{const result=await request("POST",{action:"syncW34"});setNotice(`W34 sincronizado: ${result.created} creados y ${result.updated} actualizados.`);await load();}catch(e){setError(e instanceof Error?e.message:"No fue posible sincronizar W34.");setBusy(false);}}
  ...
  return <><Head><title>Productos | Administración TECPOINT</title><meta name="robots" content="noindex,nofollow"/></Head><main className={styles.page}>
    <header><div><Link href="/admin">← Panel central</Link><p>CATÁLOGO Y CALIDAD</p><h1>Productos.</h1></div><div className={styles.headerActions}><button onClick={syncW34} disabled={busy}>{busy?"Procesando…":"Sincronizar W34"}</button><button onClick={load} disabled={busy}>{busy?"Revisando…":"Actualizar auditoría"}</button></div></header>
    {error&&<div className={styles.error}>{error}</div>}
    {notice&&<div className={styles.notice}>{notice}</div>}
    <section className={styles.metrics}>{summary&&Object.entries(summary).map(([key,value])=><button key={key} onClick={()=>setFilter(key==="total"?"all":key)}><span>{key==="total"?"Total":key==="ready"?"Listos":key==="incomplete"?"Incompletos":"Duplicados"}</span><strong>{value}</strong></button>)}</section>
