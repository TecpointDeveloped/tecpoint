...
export default function AdminProducts() {
  ...
  async function syncCatalog(action:"syncW34"|"syncW34Drafts"|"syncW35"="syncW35"){setBusy(true);setNotice("");setError("");try{const result=await request("POST",{action});setNotice(`Catálogo sincronizado: ${result.created} creados y ${result.updated} actualizados.`);await load();}catch(e){setError(e instanceof Error?e.message:"No fue posible sincronizar el catálogo.");setBusy(false);}}
  ...
  return <><Head><title>Productos | Administración TECPOINT</title><meta name="robots" content="noindex,nofollow"/></Head><main className={styles.page}>
    <header><div><Link href="/admin">← Panel central</Link><p>CATÁLOGO Y CALIDAD</p><h1>Productos.</h1></div><div className={styles.headerActions}><button onClick={()=>syncCatalog("syncW35")} disabled={busy}>{busy?"Procesando…":"Sincronizar catálogo W35"}</button><button onClick={()=>syncCatalog("syncW34Drafts")} disabled={busy}>{busy?"Procesando…":"Preparar pendientes W34"}</button><button onClick={()=>load(pagination.page)} disabled={busy}>{busy?"Revisando…":"Actualizar auditoría"}</button></div></header>
