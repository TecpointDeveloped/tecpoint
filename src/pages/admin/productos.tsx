...
export default function AdminProducts() {
  ...
  const [filter,setFilter]=useState("incomplete"); const [query,setQuery]=useState(""); const [error,setError]=useState(""); const [busy,setBusy]=useState(false);
  ...
  return <><Head><title>Productos | Administración TECPOINT</title><meta name="robots" content="noindex,nofollow"/></Head><main className={styles.page}>
    <header><div><Link href="/admin">← Panel central</Link><p>CATÁLOGO Y CALIDAD</p><h1>Productos.</h1></div><button onClick={load} disabled={busy}>{busy?"Revisando…":"Actualizar auditoría"}</button></header>
