...
export default function AdminMarketing(){
  ...
  async function archive(item:Asset){if(!window.confirm(`Retirar “${item.title}” de la página? Podrá restaurarlo después.`))return;setBusy(true);try{await request("PATCH",{...item,kind:item.kind,active:false,archived:true});await load();if("id" in draft&&draft.id===item.id)setDraft(empty(tab));}catch(e){setError(e instanceof Error?e.message:"No fue posible retirar el contenido.");}finally{setBusy(false);}}
  async function restore(item:Asset){setBusy(true);try{await request("PATCH",{...item,kind:item.kind,active:false,archived:false});await load();}catch(e){setError(e instanceof Error?e.message:"No fue posible restaurar el contenido.");}finally{setBusy(false);}}
  ...
  const shown=items.filter(item=>item.kind===tab&&!item.archived);
  const archived=items.filter(item=>item.kind===tab&&item.archived);
  return <><Head><title>Banners y promociones | TECPOINT</title><meta name="robots" content="noindex,nofollow"/></Head><main className={styles.page}>
    ...
    </form><div className={styles.list}><div><span>CONTROL DE PUBLICACIÓN</span><h2>{shown.filter(item=>item.active!==false).length} activos · {shown.filter(item=>item.active===false).length} pausados</h2></div>{shown.map(item=><article key={item.id}><Preview item={item}/><div><small>{item.active===false?"PAUSADO":"ACTIVO"} · {item.mediaType||"image"} · orden {item.sortOrder||0}</small><h3>{item.title}</h3><p>{item.subtitle}</p><div className={styles.actions}><button onClick={()=>setDraft(item)}>Editar</button><button onClick={()=>toggle(item)}>{item.active===false?"Activar":"Pausar"}</button><button className={styles.remove} onClick={()=>archive(item)}>Retirar</button></div></div></article>)}{archived.length>0&&<section className={styles.archive}><div><span>RETIRADOS</span><h2>{archived.length} piezas fuera de la página</h2></div>{archived.map(item=><article key={item.id}><Preview item={item}/><div><small>RETIRADO · {item.mediaType||"image"}</small><h3>{item.title}</h3><p>{item.subtitle}</p><div className={styles.actions}><button onClick={()=>restore(item)}>Restaurar como pausado</button></div></div></article>)}</section>}</div></section>
  </main></>;
}
...
function Preview({item}:{item:Asset}){const [failed,setFailed]=useState(false);useEffect(()=>setFailed(false),[item.id,item.imageUrl,item.videoUrl]);const source=item.imageUrl?encodeURI(item.imageUrl):"";return <div className={styles.preview} style={{position:"relative"}}>{item.mediaType==="video"&&item.videoUrl?<video src={item.videoUrl} poster={item.posterUrl} muted controls preload="metadata"/>:source&&!failed?<Image src={source} alt={item.alt||item.title} fill unoptimized sizes="210px" onError={()=>setFailed(true)}/>:<span style={{display:"grid",gap:8,textAlign:"center"}}>{failed?"No se pudo cargar la miniatura":"Sin archivo"}{item.imageUrl&&<a href={item.imageUrl} target="_blank" rel="noreferrer" style={{color:"white",textDecoration:"underline"}}>Abrir imagen</a>}</span>}</div>}
