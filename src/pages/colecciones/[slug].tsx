...
const CHILDRENS_DAY: CommercialCollection = {
  name: "Día del Niño",
  slug: "dia-del-nino",
  description:
    "Tecnología y accesorios para celebrar, aprender, jugar y compartir.",
  heroImageUrl: "",
  productSkus: [],
  keywords: [
    "audifono",
...
export const getServerSideProps: GetServerSideProps<Props> = async ({
  ...
  try {
    ...
    const campaign: CommercialCollection = {
      ...
      productSkus: Array.isArray(data.productSkus)
        ? data.productSkus.map(String)
        : [],
    ...
    const productsSnapshot = databaseName
      ? await getDocs(collection(db, databaseName))
      : null;
    const all = productsSnapshot
      ? publicCatalog([
          ...productsSnapshot.docs.map((item) => {
            ...
            return enrichProduct({
              ...
              ...value,
              fecha_agregado:
                value.fecha_agregado?.toDate?.().toISOString() || null,
            } as Product);
          }),
          ...approvedCatalogProducts(),
        ])
    ...
    const products = all
      .filter(
        (product) =>
          Boolean(getCurrentInventory(product.sku)) &&
      ...
      .filter((product) => {
        ...
        const haystack = plain(
          [
            ...
            product.sku,
            ...(Array.isArray(product.extradata?.tags)
              ? product.extradata.tags
              : []),
          ].join(" "),
        );
...
export default function CollectionLanding({ campaign, products }: Props) {
  ...
  return (
    <>
      <Head>
        <title>{campaign.name} | TECPOINT</title>
        ...
        <section id="productos" className={styles.products}>
          ...
          {products.length ? (
            ...
            <div className={styles.empty}>
              ...
              <p>
                Muy pronto encontrará aquí los productos disponibles para{" "}
                {campaign.name}.
              </p>
              <Link href="/shop">Explorar el catálogo</Link>
            </div>
          )}
        </section>
      </main>
