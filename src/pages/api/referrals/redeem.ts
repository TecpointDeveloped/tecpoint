...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!code || !code.active) return res.status(400).json({ error: "No fue posible registrar el código." });
    ...
    return res.status(500).json({ error: "No fue posible registrar el código. Inténtelo nuevamente." });
  }
}
